...
function processEvent(event) {
    try {
        ...
        if (registeredMessages[subject]) {
            listenerFound = true;
            registeredMessages[subject](data, appContext);
        }
...
(async function() {
    ...
    if (window.location) registerAllowedOrigin(window.location.origin);
})();
...
function initElementAsComponent(el) {
        ...
        Object.defineProperties(el, {
            define$: {
                value: (obj)=>{
                    ...
                    for(const prop in obj){
                        if (!prop.endsWith('$')) throw `Invalid property name '${prop}'. Property names must end with a $.`;
            ...
            initChildren$: {
                value: ()=>{
                    ...
                    if (el.componentState$ === 2) {
                        ...
                        const body = el.querySelector('body[el-is]');
                        if (head) {
            ...
            emitMessage$: {
                value: async (subject, data, target)=>{
                    if (registeredMessages[subject]) {
    ...
    async function onRender(props) {
        ...
        if (appContext.server) {
            if (Object.keys(el.state$).length) el.setAttribute('el-state', JSON.stringify(el.state$));
        }
        el.componentState$ = 2;
    }
    ...
    async function onHydrate(props) {
        ...
        if (el.hasAttribute('el-hydrate-on')) {
            el.componentState$ = 3;
            updateChildrenComponentState(el);
        ...
        for (const entry of hydrateOnComponents){
            ...
            if (hydrateOn == 'idle' || hydrateOn.startsWith('idle:')) {
                ...
                if (time) {
                    ...
                    window.requestIdleCallback(component.hydrateOnCallback$, {
                        timeout: time
                    });
                }
...
function updateChildrenComponentState(el) {
    const components = el.querySelectorAll(':scope [el-component-state]');
    for (const component of components)component.componentState$ = el.componentState$;
}
...
async function importModule(url) {
    try {
        ...
        console.log(e);
        throw `Either the requested resource or one of its dependencies was not found: ${url}`;
    }
}
    ...
    el.define$({
        ...
        onHydrate$: async (props)=>{
            ...
            for (const child of items){
                if (appContext.client && !el.ownerDocument.documentElement.hasAttribute('el-client-rendering')) Object.assign(props, child.state$.props || {});
                await child.init$(props);
            }
        },
        add$: async (props)=>{
            ...
            el.append(component);
            await component.init$(props);
            if (appContext.client && !el.ownerDocument.documentElement.hasAttribute('el-client-rendering')) component.state$.props = props;
            return component;
        }
    });
});
