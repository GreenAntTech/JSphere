...
globalThis.addEventListener('popstate', async () => {
    ...
    for (const routePath in registeredRoutes) {
        ...
        const pattern = new globalThis.URLPattern({ pathname: route.path });
...
const featureFlags = (function () {
    if (scriptHost.client) {
        ...
        if (featureFlags)
            return featureFlags.split('=')[1].split(':');
    }
    return [];
})();
...
export async function createDocument(html, ctx, config) {
    if (scriptHost.server) {
        ...
        await render(document.documentElement, config, appContext);
 ...
 * COMPONENT RENDERING
 */
export async function render(element, config, ctx) {
    //debugger;
    if (!element)
                    ...
                    if (prop == 'render') {
                        props['_' + prop] = {
                            value: async (props) => {
                                ...
                                await propObject(Object.freeze(props));
                                if (renderingStatus.atServer) {
