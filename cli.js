...
try {
    switch(cmdArgs._[0]){
        ...
        case 'start':
            ...
            break;
        case 'server':
            await processServerCmd(cmdArgs);
            break;
        case 'reset':
        ...
        if (typeof cmdArgs1.reload == 'undefined') {
            process = Deno.run({
                cmd: [
                    ...
                    `https://raw.githubusercontent.com/GreenAntTech/JSphereServer/${version}/server/server.js`
                ]
            });
        } else {
            process = Deno.run({
...
async function processServerCmd(cmdArgs10) {
    const version = cmdArgs10.v || 'main';
    const reload = cmdArgs10.reload || '';
    info(`deno run --allow-all ${cmdArgs10.reload || ''} --no-check --inspect=0.0.0.0:9229 https://raw.githubusercontent.com/GreenAntTech/JSphereServer/${version}/server/server.js`);
    let process;
    if (reload) {
        process = Deno.run({
            cmd: [
...
async function processResetCmd(cmdArgs11) {
    const domain = cmdArgs11._[1];
    if (domain) {
...
WORKDIR /${projectName}
RUN deno cache https://raw.githubusercontent.com/GreenAntTech/JSphereServer/main/server/server.js
EXPOSE 80
EXPOSE 9229
ENTRYPOINT ["deno", "run", "--allow-all", "--inspect=0.0.0.0:9229", "--no-check", "https://raw.githubusercontent.com/GreenAntTech/JSphereServer/main/server/server.js"]
`;
    return content;
}
