(() => {
  ...
  var import_mod = __require("https://deno.land/std@0.179.0/dotenv/mod.ts");
  ...
  async function processStartCmd(cmdArgs2) {
    try {
      const version = cmdArgs2.v || VERSION;
      let command;
      if (typeof cmdArgs2.reload == "undefined") {
        command = new Deno.Command(Deno.execPath(), {
          args: ["run", "--allow-all", "--no-check", "--inspect=0.0.0.0:9229", `https://raw.githubusercontent.com/GreenAntTech/JSphere/${version}/server.js`],
          stdin: "piped"
        });
      } else {
        command = new Deno.Command(Deno.execPath(), {
          args: ["run", "--allow-all", "--no-check", "--reload", "--inspect=0.0.0.0:9229", `https://raw.githubusercontent.com/GreenAntTech/JSphere/${version}/server.js`],
          stdin: "piped"
        });
      }
  ...
  async function processBuildCmd(cmdArgs2) {
    try {
      ...
      log.info(`docker build ${noCache} --pull --rm -f DockerFile -t ${projectName.toLowerCase()}:${version} .`);
      let command;
      if (typeof cmdArgs2["no-cache"] == "undefined") {
        command = new Deno.Command("docker", {
          args: ["build", "--pull", "--rm", "-f", "DockerFile", "-t", `${projectName.toLowerCase()}:${version}`, "."],
          stdin: "piped"
        });
      } else {
        command = new Deno.Command("docker", {
          args: ["build", "--no-cache", "--pull", "--rm", "-f", "DockerFile", "-t", `${projectName.toLowerCase()}:${version}`, "."],
          stdin: "piped"
        });
      }
      const child = command.spawn();
  ...
  async function processRunCmd(cmdArgs2) {
    try {
      const projectName = Deno.cwd().replaceAll("\\", "/").split("/").pop();
      const version = cmdArgs2.v || "latest";
      const http = cmdArgs2.http || "80";
      const debug = cmdArgs2.debug || "9229";
      log.info(`docker run --rm -it --mount type=bind,source=${Deno.cwd().replaceAll("\\", "/")}/,target=/${projectName} -p ${http}:80/tcp -p ${debug}:9229/tcp ${projectName.toLowerCase()}:${version}`);
      const command = new Deno.Command("docker", {
        args: ["run", "--rm", "-it", "--mount", `type=bind,source=${Deno.cwd().replaceAll("\\", "/")}/,target=/${projectName}`, "-p", `${http}:80/tcp`, "-p", `${debug}:9229/tcp`, `${projectName.toLowerCase()}:${version}`],
        stdin: "piped"
      });
      const child = command.spawn();
      child.stdin.close();
  ...
  async function processCheckoutCmd(cmdArgs2) {
    try {
      const env = await (0, import_mod.load)({ envPath: `${Deno.cwd()}/.env` });
      let repo = env.REMOTE_CONFIG;
      const provider = env.REMOTE_HOST;
      const owner = env.REMOTE_ROOT;
      const accessToken = env.REMOTE_AUTH;
      const checkout = cmdArgs2._[1].toString();
      if (checkout.startsWith(".")) {
        if (checkout != ".")
          repo = checkout;
        await cloneRepo({ provider, owner, accessToken, repo });
      } else {
        ...
        if (appConfig.packages) {
          ...
          log.error(`The application '${appId}' does not have the package '${appPackage}' registered.`);
        }
      }
    } catch (e) {
      log.critical(e.message);
  ...
  async function processCreateCmd(cmdArgs2) {
    switch (cmdArgs2._[1]) {
      case "package":
        await processCreatePackageCmd(cmdArgs2);
        break;
      case "project":
        await processCreateProjectCmd(cmdArgs2);
        break;
      default:
        log.error(`Missing 'project' or 'package' after create command.`);
    }
  }
  async function processCreatePackageCmd(cmdArgs2) {
    try {
      const packageName = cmdArgs2._[2];
      if (packageName) {
        await Deno.mkdir(`${Deno.cwd()}/${packageName}/client`, { recursive: true });
        await Deno.mkdir(`${Deno.cwd()}/${packageName}/server`, { recursive: true });
        await Deno.mkdir(`${Deno.cwd()}/${packageName}/tests`, { recursive: true });
        if (cmdArgs2["git-init"])
          await initRepo(`${Deno.cwd()}/${packageName}`);
      } else
  ...
  async function processCreateProjectCmd(cmdArgs2) {
    try {
      const projectName = cmdArgs2._[2];
      if (projectName) {
        await Deno.mkdir(`${Deno.cwd()}/${projectName}`, { recursive: true });
        const envSettings = {
          useLocalConfig: "true",
        ...
        await Deno.mkdir(`${Deno.cwd()}/${projectName}/${envSettings.localConfig}`, { recursive: true });
        await Deno.writeFile(`${Deno.cwd()}/${projectName}/${envSettings.localConfig}/server.json`, new TextEncoder().encode("{}"));
        await Deno.mkdir(`${Deno.cwd()}/${projectName}/${envSettings.localConfig}/.domains`, { recursive: true });
        await Deno.mkdir(`${Deno.cwd()}/${projectName}/${envSettings.localConfig}/.applications`, { recursive: true });
        ...
        await Deno.mkdir(`${Deno.cwd()}/${projectName}/app/client`, { recursive: true });
        await Deno.writeFile(`${Deno.cwd()}/${projectName}/app/client/index.html`, new TextEncoder().encode(getClientIndexContent()));
        await Deno.mkdir(`${Deno.cwd()}/${projectName}/app/server`, { recursive: true });
  ...
  async function processDecryptCmd(cmdArgs2) {
    try {
      const domain = cmdArgs2._[1];
      if (domain) {
        const env = await (0, import_mod.load)({ envPath: `${Deno.cwd()}/.env` });
        if (env.CRYPTO_PRIVATE_KEY) {
          const keyData = (0, import_hex.decode)(new TextEncoder().encode(env.CRYPTO_PRIVATE_KEY));
          ...
          for (const setting in settings) {
            if (typeof settings[setting] === "object" && settings[setting].encrypt) {
              const decBuffer = await crypto.subtle.decrypt(
                { name: "RSA-OAEP" },
                privateKey,
                (0, import_hex.decode)(new TextEncoder().encode(settings[setting].value))
              );
          ...
          for (const extension in domainConfig.contextExtensions) {
            const settings2 = domainConfig.contextExtensions[extension].settings;
            for (const setting in settings2) {
              if (typeof settings2[setting] === "object" && settings2[setting].encrypt) {
                const decBuffer = await crypto.subtle.decrypt(
                  { name: "RSA-OAEP" },
                  privateKey,
                  (0, import_hex.decode)(new TextEncoder().encode(settings2[setting].value))
                );
                const decData = new Uint8Array(decBuffer);
                domainFile = domainFile.replace(`"${settings2[setting].value}"`, `"${new TextDecoder().decode(decData)}"`);
              }
            }
          }
          await Deno.writeFile(`${Deno.cwd()}/${repo}/.domains/${domain}.json`, new TextEncoder().encode(domainFile));
        } else
          log.error(`Please set a value for the CRYPTO_PRIVATE_KEY environment variable.`);
      } else
        log.error(`Please provide a domain.`);
    } catch (e) {
      log.critical(e.message);
    }
  }
  async function processEncryptCmd(cmdArgs2) {
    try {
      const domain = cmdArgs2._[1];
      if (domain) {
        const env = await (0, import_mod.load)({ envPath: `${Deno.cwd()}/.env` });
        if (env.CRYPTO_PUBLIC_KEY) {
          const keyData = (0, import_hex.decode)(new TextEncoder().encode(env.CRYPTO_PUBLIC_KEY));
          ...
          for (const setting in settings) {
            if (typeof settings[setting] === "object" && settings[setting].encrypt) {
              const encBuffer = await crypto.subtle.encrypt(
                { name: "RSA-OAEP" },
                publicKey,
                new TextEncoder().encode(settings[setting].value)
              );
              const encData = new Uint8Array(encBuffer);
              domainFile = domainFile.replace(`"${settings[setting].value}"`, `"${new TextDecoder().decode((0, import_hex.encode)(encData))}"`);
            }
          }
          for (const extension in domainConfig.contextExtensions) {
            const settings2 = domainConfig.contextExtensions[extension].settings;
            for (const setting in settings2) {
              if (typeof settings2[setting] === "object" && settings2[setting].encrypt) {
                const encBuffer = await crypto.subtle.encrypt(
                  { name: "RSA-OAEP" },
                  publicKey,
                  new TextEncoder().encode(settings2[setting].value)
                );
                const encData = new Uint8Array(encBuffer);
                domainFile = domainFile.replace(`"${settings2[setting].value}"`, `"${new TextDecoder().decode((0, import_hex.encode)(encData))}"`);
              }
            }
          }
          await Deno.writeFile(`${Deno.cwd()}/${repo}/.domains/${domain}.json`, new TextEncoder().encode(domainFile));
        } else
          log.error(`Please set a value for the CRYPTO_PUBLIC_KEY environment variable.`);
      } else
        log.error(`Please provide a domain.`);
    } catch (e) {
      log.critical(e.message);
  ...
  async function processCryptoKeysCmd() {
    try {
      ...
      const publicKeyHex = new TextDecoder().decode((0, import_hex.encode)(new Uint8Array(exportedPublicKeyBuffer)));
      const privateKeyHex = new TextDecoder().decode((0, import_hex.encode)(new Uint8Array(exportedPrivateKeyBuffer)));
      const env = await (0, import_mod.load)({ envPath: `${Deno.cwd()}/.env` });
      env.CRYPTO_PUBLIC_KEY = publicKeyHex;
      env.CRYPTO_PRIVATE_KEY = privateKeyHex;
      let newEnv = "";
      for (const setting in env) {
        newEnv += `${setting}=${env[setting]}\r
`;
      }
  ...
  async function processResetCmd(cmdArgs2) {
    try {
      const domain = cmdArgs2._[1];
      const token = cmdArgs2._[2];
      if (domain) {
  ...
  async function cloneRepo(config) {
    ...
    if (config.accessToken) {
      command = new Deno.Command("git", {
        args: ["clone", `https://${config.owner}:${config.accessToken}@github.com/${config.owner}/${config.repo}.git`, path],
        stdin: "piped"
      });
    } else {
      command = new Deno.Command("git", {
        args: ["clone", `https://github.com/${config.owner}/${config.repo}.git`, path],
        stdin: "piped"
      });
    }
    const child = command.spawn();
